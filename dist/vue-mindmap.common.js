...
exports.default = function (node) {
  ...
  if (!node.url) {
	...
	if (!node.img){
		...
		content = '<img src="' + (node.img || '') + '"><div style="max-width:250px;color: #0066cc;">'+(node.content || '')+'</div>';
	}

  return '<a id="node-' + node.index + '" ' + href + ' style="padding: 10px 15px 10px 15px;">'+content+' </a>';
};
